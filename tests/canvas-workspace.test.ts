...
test("Canvas catalog has 33 unique cards and every card is searchable by its id", () => {
  assert.equal(DATA_CARD_CATALOG.length, 33)
  assert.equal(new Set(DATA_CARD_CATALOG.map((card) => card.id)).size, 33)
