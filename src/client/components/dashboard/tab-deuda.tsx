...
function RolloverBadge({ pct }: { pct: number | null }) {
  ...
  const color = pct >= 100 ? "var(--positive)" : "var(--negative)"
  return (
  ...
  return (
    <div>
      <div className="bbg-panel-header" style={{ display: "flex", justifyContent: "space-between" }}>
        ...
        <span style={{ color: "var(--text-dim)", fontWeight: 400, fontSize: 9 }}>FUENTE: argentina.gob.ar</span>
      </div>
      ...
      {error && (
        <div style={{ padding: 12, color: "var(--negative)", fontSize: 11 }}>
      ...
      {ultima && !loading && (
        <div style={{ background: "var(--bg)", border: "1px solid var(--border)", margin: 1, padding: 12 }}>
          ...
          <div style={{ display: "flex", gap: 1, flexWrap: "wrap", background: "var(--bg-elev-2)", padding: 1 }}>
            {[
              ...
              <div key={item.label} style={{ flex: "1 1 150px", background: "var(--bg-elev)", border: "1px solid var(--border)", padding: "8px 12px" }}>
                <div style={{ fontSize: 9, color: "var(--text-dim)", textTransform: "uppercase", letterSpacing: 1, marginBottom: 4 }}>
                  {item.label}
                </div>
                <div style={{ fontSize: 18, fontFamily: "var(--font-data)", fontWeight: 700, color: "var(--amber)" }}>
                  {item.value}
                </div>
                <div style={{ fontSize: 9, color: "var(--text-dim)" }}>{item.unit}</div>
              </div>
            ))}
            <div style={{ flex: "1 1 150px", background: "var(--bg-elev)", border: "1px solid var(--border)", padding: "8px 12px" }}>
              <div style={{ fontSize: 9, color: "var(--text-dim)", textTransform: "uppercase", letterSpacing: 1, marginBottom: 4 }}>
                Rollover
              </div>
              <div style={{ fontSize: 18, fontFamily: "var(--font-data)", fontWeight: 700 }}>
                <RolloverBadge pct={ultima.rollover_pct} />
              </div>
              <div style={{ fontSize: 9, color: "var(--text-dim)" }}>≥100% = financiamiento neto positivo</div>
            </div>
          </div>
            ...
            <div style={{ marginTop: 8 }}>
              ...
              <div style={{ display: "flex", gap: 6, flexWrap: "wrap" }}>
                {ultima.instrumentos.map((inst, i) => (
                  ...
                  >
                    <span style={{ color: "var(--amber)", fontWeight: 700 }}>{inst.tipo}</span>
                    <span style={{ color: "var(--text-dim)" }}>{inst.tem.toFixed(2)}% TEM</span>
                  </div>
                ))}
              </div>
            </div>
      ...
      {licitaciones.length > 0 && !loading && (
        <div style={{ marginTop: 1 }}>
          <div style={{ padding: "4px 8px", background: "var(--bg-elev-2)", fontSize: 9, color: "var(--text-dim)", textTransform: "uppercase", letterSpacing: 1, borderBottom: "1px solid var(--bg-elev-2)" }}>
            ...
            <span style={{ color: rolloverPromedio >= 100 ? "var(--positive)" : "var(--negative)" }}>
            ...
            <thead>
              <tr>
                {["Fecha", "Vencimientos ($M)", "Adjudicado ($M)", "Rollover", "Instrumentos", ""].map((h) => (
                  <th key={h} style={{ padding: "4px 8px", fontSize: 9, color: "var(--text-dim)", textAlign: h === "Fecha" ? "left" : "right", borderBottom: "1px solid var(--border)", whiteSpace: "nowrap" }}>
                    {h}
                  </th>
                ))}
              </tr>
            ...
            <tbody>
              {licitaciones.map((l, i) => (
                <tr key={i} style={{ background: i % 2 === 0 ? "var(--bg)" : "var(--bg-row-alt)" }}>
                  <td style={{ padding: "4px 8px", fontSize: 11, color: "var(--amber)" }}>{l.fecha || "—"}</td>
                  <td style={{ padding: "4px 8px", fontSize: 11, color: "#ccc", textAlign: "right", fontFamily: "var(--font-data)" }}>
                  ...
                  <td style={{ padding: "4px 8px", fontSize: 10, textAlign: "right", color: "var(--text-dim)" }}>
