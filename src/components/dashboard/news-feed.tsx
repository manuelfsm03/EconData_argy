...
export function NewsFeed({ items: propItems, title = "NEWS WIRE" }: NewsFeedProps) {
  ...
  const filteredMercury = mercuryItems.filter(item => {
      ...
      <div className="bbg-panel-header" style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: "12px" }}>
          ...
          <span style={{ color: "#555555", fontWeight: 400, fontSize: "10px" }}>
            {filteredMercury.length + items.length} items
          </span>
        </div>
      </div>
          ...
          <tbody>
            ...
            {items.map((item, i) => (
              <tr
                key={item.id}
                style={{
                  background: expandedId === item.id ? "#0a0a0a" : (i + filteredMercury.length) % 2 === 0 ? "#000000" : "#060606",
            ...
            {filteredMercury.length === 0 && items.length === 0 && (
              <tr>
                <td colSpan={5} style={{ color: "#555555", textAlign: "center", padding: "20px" }}>
                  {loading ? "CARGANDO NOTICIAS..." : "NO NEWS ITEMS AVAILABLE"}
                </td>
              </tr>
            )}
          </tbody>
